...
///
/// Uses serde's JSON tag to avoid maintaining a manual match arm per variant.
fn request_cmd_name(req: &Request) -> String {
    // Request is tagged with `#[serde(tag = "cmd", rename_all = "snake_case")]`,
    // so serializing gives `{"cmd": "<name>", ...}`.
